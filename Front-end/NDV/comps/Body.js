...
const handler = (complex, curr) =>{
  let newBody;
  fetch('http://25.78.98.95:8080/api/v1/userModules/getExercisesByComplexId/'+ complex).then(res => res.json()).then((result) =>{
    if(result.code == 200){
      console.log(result);
...
export default class  Body extends React.Component{
  ...
  async componentDidMount(){
    let curr = this;
    await axios.get('http://25.78.98.95:8080/api/v1/userModules/getComplexes').then(function (response) {
              ...
              <Container className="mb-2">
                  <Row>
                      <div className="card-deck">
                          <AddCard className ="card" bname={"exer-button-" + i} complexId={response.data.complexList[i]._id} curr={curr} titleText={response.data.complexList[i].name} descriptionText={response.data.complexList[i].description} ></AddCard>
                          <AddCard className ="card" bname={"exer-button-" + i} complexId={response.data.complexList[i+1]._id} curr={curr} titleText={response.data.complexList[i+1].name} descriptionText={response.data.complexList[i+1].description} ></AddCard>
                          <AddCard className ="card" bname={"exer-button-" + i} complexId={response.data.complexList[i+2]._id} curr={curr} titleText={response.data.complexList[i+2].name} descriptionText={response.data.complexList[i+2].description} ></AddCard>
    ...
    return(
        <Card style={{ width: '18rem' } }>
          ...
          <Card.Body>
            ...
            <Button name={props.bname} onClick={(e) => handler(props.complexId, props.curr)} variant="primary">Перейти</Button>
          </Card.Body>
        </Card>
    );
        ...
        <Jumbotron className="text-center mb-0">
            <Container>
              <h1>Готовы к тренировке?</h1>
                  <p className="lead text-muted">Мы приготовили для вас множество тренировок.</p>
                  <p className="lead text-muted">Войдите или зарегистрируйтесь, чтобы получить персональную программу тренировок.</p>
                  <p>
                  <Link href="/signin">
                    <Button className="btn-primary my-2">Войти</Button>
                  </Link>
                  <Link href="/signup">
                    <Button className="btn-secondary my-2 ml-1">Регистрация</Button>
                  </Link>
                  </p>
            </Container>
        </Jumbotron>
